...
TriangulatedObject::~TriangulatedObject() noexcept { clear(); }
...
void TriangulatedObject::draw() const {
  GL_CHECK(glBindVertexArray(vao_));
  for (const auto& mesh : mesh_data_.meshes) {
    ...
    GL_CHECK(glDrawElementsBaseVertex(
        ...
        reinterpret_cast<void*>(offset), static_cast<GLint>(mesh.base_vertex)));
  }
}

Transform& TriangulatedObject::transform() { return transform_; }

const Transform& TriangulatedObject::transform() const { return transform_; }

Vector4& TriangulatedObject::color() { return color_; }

const Vector4& TriangulatedObject::color() const { return color_; }

...
float to_radians(float degrees) { return degrees * kPi / 180.0f; }
...
void Camera::move_forward(float delta_time) {
  float distance = delta_time * movement_speed_;
  position_ += forward_ * distance;
}

void Camera::move_backward(float delta_time) { move_forward(-delta_time); }
...
void Camera::move_left(float delta_time) { move_right(-delta_time); }

void Camera::rotate_yaw(float delta_time) {
  float degrees = rotation_speed_ * delta_time;
  yaw_degrees_ += degrees;
  update_vectors();
}

void Camera::rotate_pitch(float delta_time) {
  float degrees = rotation_speed_ * delta_time;
  pitch_degrees_ = clamp_pitch(pitch_degrees_ + degrees);
  update_vectors();
}

void Camera::rotate_left(float delta_time) { rotate_yaw(-delta_time); }

void Camera::rotate_right(float delta_time) { rotate_yaw(delta_time); }

void Camera::rotate_up(float delta_time) { rotate_pitch(delta_time); }

void Camera::rotate_down(float delta_time) { rotate_pitch(-delta_time); }
...
Matrix4 Camera::projection_matrix(float width, float height) const {
  // A minimized window may temporarily report a zero-sized viewport.
  // Clamp both dimensions to keep the projection matrix finite.
  const float safe_width = std::max(width, 1.0f);
  const float safe_height = std::max(height, 1.0f);
  const float aspect_ratio = safe_width / safe_height;
...
const Vector3& Camera::forward() const { return forward_; }

const Vector3& Camera::right() const { return right_; }

const Vector3& Camera::up() const { return up_; }

float Camera::yaw() const { return yaw_degrees_; }

float Camera::pitch() const { return pitch_degrees_; }

void Camera::set_position(const Vector3& position) { position_ = position; }

...
std::optional<Validator<Action::ImportObject>::Request>
Validator<Action::ImportObject>::validate(const std::vector<Token>& tokens) {
  if (tokens.size() != 1) {
    return std::nullopt;
  }
...
std::optional<Validator<Action::ChangeColor>::Request>
Validator<Action::ChangeColor>::validate(const std::vector<Token>& tokens) {
  const auto values = parse_float_triplet(tokens, 0.0f, 255.0f);
  if (!values.has_value()) {
    return std::nullopt;
  }
  if (trunc((*values)[0]) != (*values)[0] ||
    trunc((*values)[1]) != (*values)[1] ||
    trunc((*values)[2]) != (*values)[2] ) {
    return std::nullopt;
}
  return Request{(*values)[0], (*values)[1], (*values)[2]};
}

std::optional<Validator<Action::MoveObject>::Request>
Validator<Action::MoveObject>::validate(const std::vector<Token>& tokens) {
...
std::optional<Validator<Action::RotateObject>::Request>
Validator<Action::RotateObject>::validate(const std::vector<Token>& tokens) {
...
std::optional<Validator<Action::ScaleObject>::Request>
Validator<Action::ScaleObject>::validate(const std::vector<Token>& tokens) {
...
std::optional<Validator<Action::DeleteObject>::Request>
Validator<Action::DeleteObject>::validate(const std::vector<Token>& tokens) {
  if (tokens.size() != 1) {
    return std::nullopt;
  }

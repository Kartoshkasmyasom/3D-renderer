...
namespace Renderer{
  namespace {
    void reserve_space(unsigned int num_vertices, unsigned int num_indices, MeshData& data) {
      data.positions.reserve(num_vertices);
      data.normals.reserve(num_vertices);
      data.indices.reserve(num_indices);
    }

    void count_vertices_and_indices(const aiScene* pScene, unsigned int& num_vertices, unsigned int& num_indices, MeshData& data) {
